...
class UploadToBucketTest(unittest.TestCase):
    ...
    def _patch_secrets(self, credentials_json="missing-credentials.json", project_id="proj", bucket_name="bucket"):
    ...
    def test_missing_credentials_file_returns_none_without_raising(self):
        with self._patch_secrets(credentials_json="/nonexistent/creds.json"):
    ...
    def test_client_construction_failure_is_caught(self):
        with tempfile_credentials() as creds_path:
            with self._patch_secrets(credentials_json=creds_path):
